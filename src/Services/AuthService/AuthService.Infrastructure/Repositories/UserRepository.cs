...
public class UserRepository : IUserRepository
{
    ...
    {
        _db = db; // EF DbContext injected by DI
    }
    ...
    public async Task<User?> GetByEmailAsync(string email) =>
        await _db.Users.Include(u => u.RefreshTokens).FirstOrDefaultAsync(u => u.Email == email);
    ...
    public async Task<RefreshToken?> GetRefreshTokenAsync(string token) =>
        await _db.RefreshTokens.Include(rt => rt.User).FirstOrDefaultAsync(rt => rt.Token == token);
