...
public class BillSplitDomainService : IBillSplitService
{
    ...
    public async Task<Result<BillSplitDto>> PayShareAsync(Guid splitId, Guid payerUserId, string payerEmail, string authorizationHeader)
    {
        ...
        if (split is null) return Result<BillSplitDto>.Failure("Bill split not found.");

        var normEmail = NormalizeEmail(payerEmail);
        var participant = split.Participants
            .FirstOrDefault(p => NormalizeEmail(p.Email) == normEmail);

        if (participant is null) return Result<BillSplitDto>.Failure("You are not a participant in this split.");
        if (participant.Status == ParticipantStatus.Paid) return Result<BillSplitDto>.Failure("You have already paid your share.");
        if (split.Status == BillSplitStatus.Cancelled) return Result<BillSplitDto>.Failure("This bill split has been cancelled.");
        ...
        if (payerWallet is null) return Result<BillSplitDto>.Failure("Your wallet was not found.");
        if (payerWallet.UserId != payerUserId)
            return Result<BillSplitDto>.Failure("Your wallet was not found.");
        if (payerWallet.Status == WalletService.Core.Enums.WalletStatus.Frozen) return Result<BillSplitDto>.Failure("Your wallet is frozen.");
        ...
        if (creatorWallet is null) return Result<BillSplitDto>.Failure("Creator wallet not found.");
